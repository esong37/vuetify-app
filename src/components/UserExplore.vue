        ...
        <v-row>
          <v-col cols="12" sm="6" md="4" lg="3">
            <v-card @click="playTrack(topResult)" class="music-card">
              <v-img :src="topResult.albumArt" aspect-ratio="1" class="grey--text text--darken-2"
                alt="Album Art"></v-img>
...
export default {
  ...
  computed: {
    ...mapGetters(['isAuthenticated']), // using vuex
  },
  watch: {
  ...
  methods: {
    async fetchSearchResults() {
      ...
      if (!this.isAuthenticated) {
        this.$router.push('/login'); // if not login, redirect
        return;
      }
      ...
      if (this.searchQuery) {
        try {
          ...
          if (matchingTrack) {
            ...
            this.topResult = {
              ...matchingTrack,
              src: `http://localhost:3000/api/stream/${matchingTrack.track_id}`, 
